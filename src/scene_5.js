scene_5 = {
	...
	calculateScene: function(diff){		
		world.camera.position = [0,10,-50];
		world.camera.rotation = [30,-60,0];
		this.time += diff;
		if(this.time > 300){
			this.addCubes();
			this.time = 0;
		...
		world.camera.dolly[0] = 30 + this.camera_pos;
		if(this.camera_pos > -this.x_offset){
			this.camera_pos -= (0.001 * diff) * ((this.x_offset+this.camera_pos)/5);
		}
